...
const MyBlogList = ({ posts }: { posts: any }) => {
    return (
        <div className="mt-8 grid grid-cols-1 gap-8 px-2 sm:grid-cols-2 lg:grid-cols-4">
            {posts.map((post) => (
                <div key={post.id} className="bg-background flex flex-col items-start overflow-hidden rounded-lg shadow transition hover:shadow-lg">
                    <div className="relative aspect-video w-full">
                        <img
                            src={`/assets/images/posts/thumb/${post.images && post.images[0].image}`}
                            alt={post.title}
                    ...
                    <div className="flex flex-1 flex-col items-start justify-between p-2">
                        <div>
                            <p className="mb-2 line-clamp-1 text-sm text-gray-500">{post.post_date}</p>
                            <h3 className="mb-3 line-clamp-2 text-xl font-semibold">{post.title}</h3>
                            <p className="mb-4 line-clamp-3 text-sm text-gray-600">{post.short_description}</p>
                        </div>
