                ...
                <div className="section-container mx-auto flex items-center justify-between px-4">
                    ...
                    <div className="flex items-center gap-3">
                        <Link href={`/pages/DONATE`} prefetch>
                            <button className="flex items-center gap-2 rounded-full bg-[#e62129] px-5 py-2 text-base font-bold text-white transition-all hover:bg-red-700 active:scale-95">
                                <Gift size={18} />
            ...
            <footer className="bg-muted dark:bg-background mt-12 border-t py-10 transition-colors duration-300">
                <div className="section-container mx-auto grid grid-cols-1 gap-10 px-4 md:grid-cols-2 lg:grid-cols-3">
                    <div className="space-y-6">
                        <h1 className="text-3xl font-bold text-[#e62129]">{application_info?.name || 'CRC News'}</h1>
                    ...
                    <div>
                        <h5 className="mb-6 text-lg font-bold text-[#e62129]">Quick Links</h5>
                        <ul className="space-y-4">
                            {page_links.map((item: any) => (
                                <li key={item.position_code}>
                                    <Link
                                        href={`/pages/${item.position_code}`}
                                        className="group text-muted-foreground flex items-center gap-2 text-base font-bold transition-colors hover:text-[#e62129]"
                                    >
                    ...
                    <div>
                        <h5 className="mb-4 text-lg font-bold text-[#e62129]">Contact Information</h5>
                ...
                <div className="section-container border-border text-muted-foreground mx-auto mt-10 border-t px-4 pt-8 text-center text-base font-bold">
...
function NavItem({ children, href, active }: any) {
    return (
        <a
            href={href}
            className={`font-bold transition-colors duration-200 hover:text-[#e62129] ${active ? 'border-b-2 border-[#e62129] text-[#e62129]' : 'text-slate-500 dark:text-slate-400'}`}
        >
            {children}
        </a>
    );
